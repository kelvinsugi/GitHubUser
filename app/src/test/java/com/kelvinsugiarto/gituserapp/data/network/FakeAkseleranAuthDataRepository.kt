...
class FakeAkseleranAuthDataRepository constructor(
    private val akseleranLoginApi: AkseleranLoginApi) : AkseleranAuthUseCase {
    ...
    override suspend fun login(credentialModel: CredentialModel): DataResult<Any> {
        try{
            ...
            return when {
                ...
                response.code() == 401 -> {
                    val unauthorizedResponse = Gson().fromJson(response.errorBody()!!.charStream(),
                        UnauthorizedResponse::class.java)
